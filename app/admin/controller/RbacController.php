...
class RbacController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $data = Db::name('role')
            ->alias('a')
            ->join('__ROLE__ b','a.parent_id = b.id','left')
            ->order(["list_order" => "ASC", "id" => "DESC"])
            ->field('a.*,b.name as parent_name')
            ->select();
        $data = $this->list_tree($data);
        $this->assign("roles", $data);
        return $this->fetch();
    }


    private function list_tree($data,$parent_id = 0,$deep = 0){
        static $ret = [];
        foreach($data as $val){
            if($parent_id == $val['parent_id']){
                ...
                $val['deep'] = $_deep;
                array_push($ret,$val);
                $this->list_tree($data,$val['id'],++$_deep);
            }
        }
        return $ret;
    }
    ...
    public function roleAdd()
    {
        ...
        $this->assign('role_tree',$this->createTree());

        return $this->fetch();
    }

    private function createTree($selectId = 0,$currentCid = 0){
        $where = [];
        if (!empty($currentCid)) {
            $where['id'] = ['neq', $currentCid];
        }
        $categories =    Db::name('role')->where($where)->select();
    ...
    public function roleEdit()
    {
        ...
        $this->assign('role_tree',$this->createTree($data['parent_id'],$id));
        return $this->fetch();
    }
    ...
    public function roleEditPost()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $count = Db('role')
                ...
                ->where('recommend_code',$data['recommend_code'])
                ->count();
            if($count > 0){
                $this->error('推荐码重复');
            }
